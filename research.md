...
Practically, our research involves working with a variety of organisms and data types. One component of our research involves the fine-grained investigation of speciation in individual lineage pairs, another involves large-scale data syntheses and comparative analyses, and yet another component is purely theoretical.

Conceptually, our work is organized around a basic premise, which is that for biodiversity to arise and accumulate, two processes must occur. First, lineages must split into two or more descendants (i.e. they must (a) initiate speciation and (b) resist sliding back into a hybrid swarm), and second, descendant lineages must establish overlapping ranges. The second idea arises from the fact that speciation is often an allopatric process; thus, following a speciation event, for subsequent rounds of allopatric speciation to occur, daughter species must expand geographically to attain range sizes that can bear further subdivision. In nature, this often that means lineages must overlap with their close relatives. Speciation and the establishment of sympatry are then two critical rate-limiting steps in the diversification cycle; the primary focus of our work is to better understand the factors affecting both processes. 

Several competing hypothesis have been put forth regarding these factors, and debates over their relative importance have raged in the literature for decades (the history of speciation research is indeed one of strong opinions). Frequently, these disagreements have fallen along the lines of differences in study system, and general insights have proven elusive. But by generating new and creative quantiative approaches, we can leverage "big data" to pick up where rhetoric alone leaves-off in the attempt to break some long-standing stalemates. Thus, two major tasks of our work are data syntheses and the development of statistical models. Our goal is to draw <b><i> general </i></b> inferences regarding (1) the mechanisms driving (or halting) progress toward the origin of new species and (2) how spatial variation in those mechanisms explains the unequal distribution of biodiversity across the globe.

While much of the work described below deals with trait data, much of our current research is concerned with genomic differentiation between speciating taxa and the factors affecting its dynamics when lineages come into secondary contact. 

<!--<Finally, while nature can be profoundly idiosyncratic, it is ultimately necessary to generalize when attempting to explain broader-scale patterns of diversity. For this reason... While the first requirement is self-evident, the second follows from the fact that speciation is largely an allopatric process, and allopatry is often achieved when a species’ range is split into two or more isolated subdivisions (i.e. “vicariance”). But vicariance alone is ultimately limited in its ability to generate biodiversity, as a range can only be split so many times into smaller and smaller parts. For diversification to continue, daughter species must expand geographically to attain range sizes that can bear further subdivision. In nature, this often that means lineages must overlap with their close relatives. -->
